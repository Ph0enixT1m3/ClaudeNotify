...
Что установится:
- `ClaudeNotify.app` — нативные уведомления macOS
- Хуки в `~/.claude/settings.json`
- Скилл `/setup-project`
- `.claude/settings.json` в текущем проекте — **автономные разрешения** (без диалогов при каждой команде)

После установки:
- Если появился запрос разрешений на уведомления — нажми **Разрешить**
- **System Settings → Notifications → Claude Code → Alerts**
- Перезапусти сессию Claude Code — разрешения вступят в силу
